...
void AdminService::AddDress(std::string size, std::string color, size_t price, size_t quantity, std::string photgraph)
{
	...
	try {
		...
		for (size_t i = 0; i < this->repo.GetSize(); ++i)
			if (this->repo.GetArray()[i] == newDress) duplicate = true;
...
void AdminService::RemoveDress(std::string photograph)
{
	...
	if (this->repo.GetArray()[pos].GetQuantity() != 0)
...
void AdminService::UpdateDressColor(std::string photograph, std::string newColor)
{
	...
	bool duplicate = false;
	for (size_t i = 0; i < this->repo.GetSize(); ++i)
		if (this->repo.GetArray()[i].GetColor() == newColor) duplicate = true;
...
std::vector<Dress> AdminService::getAll()
{
...
void AdminServicesTests::TestConstructors()
{
	...
	assert(s2.repo.GetSize() == s2.GetRepo().GetSize());
}
...
void AdminServicesTests::TestAdd()
{
	...
	assert(s2.repo.GetArray()[0].GetSize() == "xs");
	assert(s2.repo.GetArray()[0].GetColor() == "black");
	assert(s2.repo.GetArray()[0].GetPrice() == 33);
	assert(s2.repo.GetArray()[0].GetQuantity() == 33);
	assert(s2.repo.GetArray()[0].GetPhotograph() == "https://google.ro");

	s2.AddDress("xs", "white", 33, 33, "https://google.ro");
	assert(s2.repo.GetArray()[1].GetSize() == "xs");
	assert(s2.repo.GetArray()[1].GetColor() == "white");
	assert(s2.repo.GetArray()[1].GetPrice() == 33);
	assert(s2.repo.GetArray()[1].GetQuantity() == 33);
	assert(s2.repo.GetArray()[1].GetPhotograph() == "https://google.ro");

	bool exceptionThrown = false;
	
	try {
		s2.AddDress("xs", "black", 0, 0, "https://google.ro");
	}
	catch (std::exception) {
...
void AdminServicesTests::TestRemove()
{
	...
	assert(s2.repo.GetArray()[0].GetSize() == "xs");
	assert(s2.repo.GetArray()[0].GetColor() == "black");
	assert(s2.repo.GetArray()[0].GetPrice() == 200);
	assert(s2.repo.GetArray()[0].GetQuantity() == 5);
	assert(s2.repo.GetArray()[0].GetPhotograph() == "https://google.ro");
	...
	s2.RemoveDress("black");
	assert(s2.repo.GetArray()[0].GetSize() == "xs");
	assert(s2.repo.GetArray()[0].GetColor() == "white");
	assert(s2.repo.GetArray()[0].GetPrice() == 150);
	assert(s2.repo.GetArray()[0].GetQuantity() == 8);
	assert(s2.repo.GetArray()[0].GetPhotograph() == "https://google.ro");
...
void AdminServicesTests::TestUpdate()
{
	...
	assert(s2.repo.GetArray()[0].GetColor() == "rr");
	...
	assert(s2.repo.GetArray()[0].GetSize() == "s");
	assert(s2.repo.GetArray()[0].GetPrice() == 3333);
	assert(s2.repo.GetArray()[0].GetQuantity() == 333);
	assert(s2.repo.GetArray()[0].GetPhotograph() == "https://dress.ro");
