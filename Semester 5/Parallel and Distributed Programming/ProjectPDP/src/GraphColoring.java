...
public class GraphColoring {
    ...
    private static int[] graphColoringRecursive(int nodeId, Graph graph, int colorsNumber, int[] codes, int mpiRank, int mpiSize, int power){
        ...
        int coefficient = (int) Math.pow(colorsNumber,power);   // colorsN^power range of color codes/process
        int colorCode = 0;
        int destination = mpiRank + coefficient * (colorCode + 1);  // initial destination
        ...
        for (int currentColorCode = 1; currentColorCode < colorCode; currentColorCode++){
            ...
            int[] data = new int[]{mpiRank, nextNode, nextPower};  // current state
            MPI.COMM_WORLD.Send(data,0,data.length,MPI.INT,destination,0);

            int[] nextColorCodes = Arrays.copyOf(codes,codes.length);  // current color codes array
            nextColorCodes[nextNode] = currentColorCode;

            // sends updated color codes array
    ...
    public static void graphColoringChild(int mpiRank, int mpiSize, Graph graph, int colorsNumber) {
        ...
        // receive data
        int[] data = new int[3]; //current coloring attempt
        ...
        int[] codes = new int[nodesNumber]; // current coloring info
        ...
        // after recursive done send data to parent
